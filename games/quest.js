    ...
    create() {
      ...
      function updateHazards(dt) {
        ...
        for (let i = hazards.length - 1; i >= 0; i--) {
          ...
          else if (h.type === "spit" || h.type === "web") { h.t += dt; h.x += h.vx * dt; h.y += h.vy * dt; if (h.t > h.dur) gone = true; else if (Math.hypot(p.x - h.x, p.y - h.y) < frogR() * 1.1) { if (h.type === "spit") takeHit(h.dmg, h.x - h.vx, h.y - h.vy); else { p.slow = 2200; takeHit(1, h.x - h.vx * 100, h.y - h.vy * 100); fx.push({ kind: "txt", x: p.x, y: p.y - frogR() * 2.6, t: 0, dur: 700, text: "webbed!", col: "#c9c3ff" }); } gone = true; } }
